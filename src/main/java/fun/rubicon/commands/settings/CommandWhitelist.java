...
public class CommandWhitelist extends CommandHandler {
    ...
    public CommandWhitelist() {
        super(new String[] {"wl", "whitelist"}, CommandCategory.SETTINGS, new PermissionRequirements(PermissionLevel.WITH_PERMISSION, "command.whitelist"), "Whitelist one or more channel for bot commands", "whitelist list/add/remove", false);
    }
    ...
    private void executeList(String[] args, Member member, Guild guild, TextChannel textChannel) {
        if(whitelistEnabled(guild)) {
            List<String> channeldIDs = Arrays.asList(RubiconBot.getMySQL().getGuildValue(guild, "whitelist").split(","));
            StringBuilder channels = new StringBuilder();
            channeldIDs.forEach(id -> {
                try{
            ...
            SafeMessage.sendMessage(textChannel, EmbedUtil.info("Blacklisted Channels", "Whitelist mode: `" + String.valueOf(whitelistEnabled(guild)).replace("true", "enabled").replace("false", "disabled") + "`\nChannels: `" + channels.toString() + "`").build());
        }else
            SafeMessage.sendMessage(textChannel, EmbedUtil.info("Blacklisted Channels", "Whitelist mode: `" + String.valueOf(whitelistEnabled(guild)).replace("true", "enabled").replace("false", "disabled") + "`").build());
    }
