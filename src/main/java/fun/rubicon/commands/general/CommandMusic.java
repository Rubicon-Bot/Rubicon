...
import fun.rubicon.core.music.AudioInfo;
        ...
        switch (args.length) {
            ...
            case 2:
                switch (args[0].toLowerCase()) {
                    ...
                    // case "channel":
                    //    if (getPermissionLevel() > 0) return;
                    //    if (args.length < 2) {
                    ...
                    case "skip":
                        if (isCurrentDj(event.getMember()) || isDj(event.getMember())) {
                            ...
                            event.getTextChannel().sendMessage(new EmbedBuilder().setColor(Color.red).setDescription(":warning:  Sorry, but you need to be a DJ to skip tracks!").build()).queue();
                        }
                        break;
                    ...
                    case "endless":
                        ...
                        event.getTextChannel().sendMessage(new EmbedBuilder().setColor(Color.green).setDescription(":repeat:  Endless mode activated.").build()).queue();
...
//
            ...
            default:
                ...
                switch (args[0].toLowerCase()) {
                    ...
                    case "play":
                        if (args.length <= 1) {
                            event.getTextChannel().sendMessage(new EmbedBuilder().setColor(Color.red).setDescription(":warning:  Please include a valid source.").build()).queue();
                        } else {
